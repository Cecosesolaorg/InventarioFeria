...
<div id="calcModal" class="modal">
    <div class="modal-content premium-calculator">
        ...
        <div class="calc-grid">
            ...
            <button class="calc-btn math-op" data-op="+">+</button>
            <button class="calc-btn spec-btn" data-op="M">M</button>
    ...
    const CalcApp = {
        ...
        performCalc: function () {
            try {
                ...
                let res = eval(expr);
                if (!isNaN(res)) {
                    let isOpOnTotal = this.currentExpression.includes("M");
                    let old = this.tempTotal;
                    this.tempTotal = isOpOnTotal ? res : old + res;

                    let step = isOpOnTotal
                        ? `(${this.currentExpression.replace(/M/g, old)})=${this.tempTotal}`
                        : `+${this.currentExpression}=${this.tempTotal}`;

                    this.tempHistory = (this.tempHistory === "0" || this.tempHistory === "")
                        ? (isOpOnTotal ? step : `${this.currentExpression}=${this.tempTotal}`)
