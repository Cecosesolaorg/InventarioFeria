...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (name && displayName) {
        displayName.textContent = `SISTEMA: ${name} ${lastName || ''}`;
    }
        ...
        if (areas.length === 0) {
            assignmentList.innerHTML = `
                <div style="text-align:center; padding: 20px;">
                    ...
                    <p style="color:#888; font-size:0.9rem; margin: 15px 0;">ESTAMOS TRABAJANDO EN ELLO ESTA FUNCION POR LOS MOMENTOS NO ESTADISPONIBLE.</p>
    ...
    // --- LÓGICA DE INGRESO A PASILLOS (MODAL PERSONALIZADO) ---
    const aisleLinks = document.querySelectorAll('.link-card:not(.card-disabled)');
    aisleLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            confirmBtn.onclick = () => {
                ...
                if (name) {
                    ...
                    alert("Por favor, ingresa el nombre del compañero.");
                }
            };
